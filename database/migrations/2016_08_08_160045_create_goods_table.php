...
class CreateGoodsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('goods', function (Blueprint $table) {
            ...
            $table->string('goods_barcode',60)->comment('商品条码');
            ...
            $table->string('keywords',255)->default(0)->comment('关键字');
            $table->text('goods_desc')->nullable()->comment('商品详情');
            $table->string('goods_brief',255)->comment('简明信息');
            $table->unsignedInteger('img_id')->default(0)->comment('封面图');
            $table->string('goods_thumb',255)->comment('商品缩略图');
            $table->string('goods_img',255)->comment('商品图');
            $table->string('original_img',255)->comment('商品原图');
            $table->boolean('is_real')->default(true)->comment('实际商品');
            $table->string('extension_code',30)->default(true)->comment('扩展代码');
            ...
            $table->string('seller_note',255)->comment('商家备注');
